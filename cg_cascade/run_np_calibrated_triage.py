#!/usr/bin/env python
"""Source-matched NP/Clopper-Pearson calibration for cost-aware triage triage.
...
def main() -> None:
    model = joblib.load(MODELS / "triage_tfidf_logreg_promptshield.pkl")
    test_df = pd.read_parquet(DATA / "promptshield_test.parquet")
    natural = load_json(RESULTS / "triage_openai_promptshield_test_natural3000.json")
    balanced = load_json(RESULTS / "triage_openai_promptshield_test_balanced2000.json")
    ...
    payload = {
        "experiment": "triage_np_calibrated_triage",
    ...
    out_json = RESULTS / "triage_np_calibrated_triage.json"
    ...
    out_md = RESULTS / "triage_np_calibrated_triage.md"
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
