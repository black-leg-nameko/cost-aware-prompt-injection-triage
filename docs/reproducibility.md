...
- Adaptive stress testing: run `cg_cascade/run_iwsec_adaptive_padding_attack.py`
  after the main LLM judge outputs exist. Then run
  `cg_cascade/run_iwsec_adaptive_padding_e2e.py`,
  `cg_cascade/run_iwsec_sparse_mix_audit_e2e.py`,
  `cg_cascade/run_iwsec_quarantine_session_e2e.py`, and
  `cg_cascade/run_iwsec_sparse_adaptive_defense_experiments.py` for the
  end-to-end audit, quarantine, density-gate, and padding-variant analyses.
  `cg_cascade/run_iwsec_probabilistic_audit_simulation.py` gives the
